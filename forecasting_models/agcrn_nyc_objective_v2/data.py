...
TARGET_MODES = ("raw", "log1p", "seasonal_residual", "log1p_seasonal_residual")
...
class ObjectiveBundleData:
    ...
    def inverse_model_tensor(self, values: torch.Tensor, baselines: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_mode == "log1p_seasonal_residual":
            baseline_log = torch.log1p(baselines.to(device=values.device, dtype=values.dtype))
            return torch.expm1(unscaled + baseline_log).clamp_min(0)
...
def transform_targets(
    ...
    *,
    ...
    if target_mode == "log1p_seasonal_residual":
        return (np.log1p(raw_targets) - np.log1p(baselines)).astype(np.float32), baselines
