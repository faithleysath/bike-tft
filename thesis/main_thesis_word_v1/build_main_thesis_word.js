...
const SOURCE_MD = path.join(ROOT, "thesis", "main_thesis_v1", "thesis_draft.md");
const FIGURE_DIR = path.join(ROOT, "thesis", "main_thesis_v1", "figures");
...
const OUT_DOCX = path.join(OUT_DIR, "main_thesis_word_v1_final.docx");
const OUT_PDF = path.join(OUT_DIR, "main_thesis_word_v1_final.pdf");
const DRAFT_DOCX = path.join(WORK_DIR, "main_thesis_word_v1_pass1.docx");
const DRAFT_PDF = path.join(WORK_DIR, "main_thesis_word_v1_pass1.pdf");
const REPORT = path.join(OUT_DIR, "visual_check_report.md");
const MANIFEST = path.join(OUT_DIR, "main_thesis_word_v1_manifest.json");
...
function inlineRuns(text, opts = {}) {
  ...
  for (const part of parts) {
    ...
    if (part.startsWith("`") && part.endsWith("`")) {
      ...
      runs.push(run(cleanInlineText(part), { size, font, bold: opts.bold }));
    }
  }
  return runs;
}
...
function formulaParagraph(tex) {
  return center(cleanInlineText(tex), {
    size: 22,
    font: FONT_EN,
...
function parseBodyElements(lines) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (line.startsWith("$$")) {
      ...
      elements.push({ type: "formula", text: formulaLines.join(" ") });
      continue;
    }
    ...
    if (h2) {
      flushParagraph();
      elements.push({ type: "heading", level: 1, text: h2[1].trim() });
      continue;
    }
...
function tocSection(headings, tocPageMap) {
  ...
  for (const h of headings) {
    const page = tocPageMap[h.text] || "";
    const leftIndent = h.level === 1 ? 0 : h.level === 2 ? 420 : 840;
    const font = h.level === 1 ? FONT_HEI : FONT_CN;
    const size = h.level === 1 ? 28 : 24;
    children.push(
      paragraph([
        run(h.text, { size, font, bold: h.level === 1 }),
        run("\t", { size, font }),
        run(String(page), { size, font }),
      ], {
...
function bodySection(elements) {
  ...
  for (const element of elements) {
    if (element.type === "heading") {
      ...
      children.push(formulaParagraph(element.text));
  ...
  return {
    properties: {
      type: SectionType.NEXT_PAGE,
      page: { ...PAGE.portrait, pageNumbers: { start: 1, formatType: "decimal" } },
    },
...
function collectChecks(pdfPath, tocDerivation, headings) {
  ...
  return {
    ...
    hasAppendix: allText.includes(normalizeMatchText("附录A 主要实验结果来源")) && allText.includes(normalizeMatchText("附录B 主要代码目录")),
...
function writeReport(checks, tocMap) {
  const lines = [
    ...
    `- Appendix detected: ${checks.hasAppendix ? "PASS" : "FAIL"}`,
