...
const OUT_DOCX = path.join(OUT_DIR, "main_thesis_word_v18_final.docx");
const OUT_PDF = path.join(OUT_DIR, "main_thesis_word_v18_final.pdf");
const DRAFT_DOCX = path.join(WORK_DIR, "main_thesis_word_v18_pass1.docx");
const DRAFT_PDF = path.join(WORK_DIR, "main_thesis_word_v18_pass1.pdf");
const REPORT = path.join(OUT_DIR, "visual_check_report.md");
const MANIFEST = path.join(OUT_DIR, "main_thesis_word_v18_manifest.json");
...
function coverSection(logoBuffer, wordmarkBuffer) {
  ...
  children.push(
    ...
    coverEntry("学生姓名：", AUTHOR),
    coverEntry("学    号：", STUDENT_ID),
    coverEntry("专    业：", MAJOR),
    coverEntry("学    院：", COLLEGE),
    coverEntry("指导教师：", ADVISOR),
    emptyParagraph(360),
...
function coverEntry(label, value, opts = {}) {
  const size = opts.size || 30;
  const minChars = opts.minChars || 18;
  const padding = Math.max(minChars - String(value).length, 2);
  const underlinedValue = `${value}${" ".repeat(padding)}`;
  return paragraph([
    run(label, { size, font: opts.labelFont || FONT_CN, bold: opts.boldLabel }),
    run(underlinedValue, { size, font: opts.valueFont || FONT_CN, bold: opts.boldValue, underline: true }),
  ], {
    alignment: AlignmentType.LEFT,
    indent: { left: 1500 },
    spacing: { before: 0, after: opts.after || 150, line: 340 },
  });
}
...
function declarationSection() {
  return {
    ...
    children: [
      ...
      emptyParagraph(180),
      statementSignatureLine("论文作者签名：____________         签字日期：______________"),
      emptyParagraph(360),
      ...
      emptyParagraph(160),
      statementSignatureLine("论文作者签名：______________            签字日期：_______________"),
      statementSignatureLine("指导教师签名：______________            签字日期：_______________"),
    ],
  };
}
...
function writeReport(checks, tocMap) {
  const lines = [
    ...
    "- V18 follows the Attachment 4 front-matter order: cover, declaration/authorization, table of contents, Chinese abstract, English abstract, and body.",
    "- V18 aligns the declaration and authorization pages to the Attachment 4 sample: red centered headings, KaiTi body text, left-aligned signature lines, and the prescribed authorization wording.",
    "- V18 keeps the cover rebuilt against the Attachment 4 front-page sample, including the red NUIST wordmark and field layout.",
    "- V18 keeps the Word automatic TOC field with cached page entries for PDF preview.",
    "- V18 tightens the level-1 TOC style to black SimHei four-point text and left-aligns references to avoid stretched English spacing.",
    "- V18 removes the short appendix section and keeps signature lines blank because signatures are not required for this pass.",
