...
const SOURCE_MD = path.join(ROOT, "thesis", "main_thesis_v9", "thesis_draft.md");
const FIGURE_DIR = path.join(ROOT, "thesis", "main_thesis_v9", "figures");
...
const OUT_DOCX = path.join(OUT_DIR, "main_thesis_word_v9_final.docx");
const OUT_PDF = path.join(OUT_DIR, "main_thesis_word_v9_final.pdf");
const DRAFT_DOCX = path.join(WORK_DIR, "main_thesis_word_v9_pass1.docx");
const DRAFT_PDF = path.join(WORK_DIR, "main_thesis_word_v9_pass1.pdf");
const REPORT = path.join(OUT_DIR, "visual_check_report.md");
const MANIFEST = path.join(OUT_DIR, "main_thesis_word_v9_manifest.json");
...
function writeReport(checks, tocMap) {
  const lines = [
    ...
    "- V9 keeps signature lines blank because signatures are not required for this pass.",
