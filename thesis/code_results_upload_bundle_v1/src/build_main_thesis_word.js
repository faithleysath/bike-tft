...
const SOURCE_MD = path.join(ROOT, "thesis", "main_thesis_v15", "thesis_draft.md");
const FIGURE_DIR = path.join(ROOT, "thesis", "main_thesis_v15", "figures");
...
const OUT_DOCX = path.join(OUT_DIR, "main_thesis_word_v15_final.docx");
const OUT_PDF = path.join(OUT_DIR, "main_thesis_word_v15_final.pdf");
const DRAFT_DOCX = path.join(WORK_DIR, "main_thesis_word_v15_pass1.docx");
const DRAFT_PDF = path.join(WORK_DIR, "main_thesis_word_v15_pass1.pdf");
const REPORT = path.join(OUT_DIR, "visual_check_report.md");
const MANIFEST = path.join(OUT_DIR, "main_thesis_word_v15_manifest.json");
...
function writeReport(checks, tocMap) {
  const lines = [
    ...
    "- V15 rebuilds the cover against the Attachment 4 front-page sample, including the red NUIST wordmark and field layout.",
    "- V15 keeps the Word automatic TOC field with cached page entries for PDF preview.",
    "- V15 keeps justified reference entries and paragraph word wrap for long English words.",
    "- V15 removes the short appendix section and keeps signature lines blank because signatures are not required for this pass.",
