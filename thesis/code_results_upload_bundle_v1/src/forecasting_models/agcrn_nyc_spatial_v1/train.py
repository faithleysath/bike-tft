"""Train spatial-prior AGCRN on the NYC Citi Bike bundle."""
...
from .model import SpatialAGCRN, SpatialAGCRNConfig, build_distance_support


DEFAULT_BUNDLE = Path("dataset/preprocessing/processed/nyc_top883/nyc_agcrn_bundle.npz")
DEFAULT_STATION_STATIC = Path("dataset/preprocessing/processed/nyc_top883/nyc_station_static_features.csv")
TARGET_NAMES = ("dep", "arr")


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments."""
    parser = argparse.ArgumentParser(description="Train NYC spatial-prior AGCRN for dep+arr forecasting.")
    parser.add_argument("--bundle", default=DEFAULT_BUNDLE.as_posix())
    parser.add_argument("--station-static", default=DEFAULT_STATION_STATIC.as_posix())
    parser.add_argument("--output-dir", default="forecasting_models/agcrn_nyc_spatial_v1/runs/default")
    ...
    parser.add_argument("--spatial-top-k", type=int, default=20)
    parser.add_argument("--spatial-sigma-km", type=float, default=None)
    parser.add_argument("--spatial-init-mix", type=float, default=0.5)
    parser.add_argument("--spatial-mode", choices=("fused", "separate"), default="fused")
    ...
    if args.spatial_mode == "fused" and args.support_count < 2:
        parser.error("--support-count must be at least 2 for fused mode")
    if args.spatial_mode == "separate" and args.support_count < 3:
        parser.error("--support-count must be at least 3 for separate mode")
    if args.spatial_top_k < 1:
        parser.error("--spatial-top-k must be positive")
    if args.spatial_sigma_km is not None and args.spatial_sigma_km <= 0:
        parser.error("--spatial-sigma-km must be positive")
    if not 0.0 < args.spatial_init_mix < 1.0:
        parser.error("--spatial-init-mix must be in (0, 1)")
    return args
...
def main() -> int:
    """Train and evaluate the spatial-prior NYC AGCRN model."""
    try:
        args = parse_args()
        set_seed(args.seed)
        ...
        spatial_support, spatial_metadata = build_distance_support(
            args.station_static,
            data.station_ids,
            top_k=args.spatial_top_k,
            sigma_km=args.spatial_sigma_km,
        )
        config = SpatialAGCRNConfig(
            ...
            spatial_init_mix=args.spatial_init_mix,
            spatial_mode=args.spatial_mode,
        )
        model = SpatialAGCRN(config, spatial_support).to(device)
