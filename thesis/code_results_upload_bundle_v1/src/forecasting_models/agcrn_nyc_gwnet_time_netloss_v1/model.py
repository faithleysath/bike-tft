...
class GraphWaveNetConfig:
    ...
    graph_order: int = 2
    dropout: float = 0.1
    future_time_dim: int = 7
    future_time_embed_dim: int = 16
...
class GraphWaveNet(nn.Module):
    ...
    def __init__(self, config: GraphWaveNetConfig, relation_supports: torch.Tensor) -> None:
        ...
        self.future_time_encoder = nn.Sequential(
            nn.Linear(config.future_time_dim, config.future_time_embed_dim),
            nn.ReLU(),
            nn.Linear(config.future_time_embed_dim, config.future_time_embed_dim),
            nn.ReLU(),
        )
        self.horizon_readout_1 = nn.Conv2d(
            config.end_channels + config.future_time_embed_dim,
            config.end_channels,
            kernel_size=(1, 1),
        )
        self.horizon_readout_2 = nn.Conv2d(config.end_channels, config.output_dim, kernel_size=(1, 1))
    ...
    def forward(self, source: torch.Tensor, future_time_features: torch.Tensor) -> torch.Tensor:
        """Predict future departures and arrivals.

        Args:
            source: Tensor shaped [batch, lag, nodes, input_dim].
            future_time_features: Tensor shaped [batch, horizon, future_time_dim].
        ...
        if future_time_features.shape[1] != self.config.horizon or future_time_features.shape[2] != self.config.future_time_dim:
            raise ValueError(
                f"Expected future_time_features shape [B,{self.config.horizon},{self.config.future_time_dim}], "
                f"got {tuple(future_time_features.shape)}"
            )
        ...
        time_embedding = self.future_time_encoder(future_time_features.to(device=hidden.device, dtype=hidden.dtype))
        batch_size = hidden.shape[0]
        hidden = hidden.unsqueeze(1).expand(-1, self.config.horizon, -1, -1)
        time_embedding = time_embedding.unsqueeze(-1).expand(-1, -1, -1, self.config.num_nodes)
        conditioned = torch.cat([hidden, time_embedding], dim=2)
        conditioned = conditioned.reshape(
            batch_size * self.config.horizon,
            self.config.end_channels + self.config.future_time_embed_dim,
            self.config.num_nodes,
            1,
        )
        output = F.relu(self.horizon_readout_1(conditioned))
        output = self.horizon_readout_2(output).squeeze(-1)
        output = output.reshape(batch_size, self.config.horizon, self.config.output_dim, self.config.num_nodes)
