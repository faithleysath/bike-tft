...
const SOURCE_MD = path.join(ROOT, "thesis", "main_thesis_v7", "thesis_draft.md");
const FIGURE_DIR = path.join(ROOT, "thesis", "main_thesis_v7", "figures");
...
const OUT_DOCX = path.join(OUT_DIR, "main_thesis_word_v8_final.docx");
const OUT_PDF = path.join(OUT_DIR, "main_thesis_word_v8_final.pdf");
const DRAFT_DOCX = path.join(WORK_DIR, "main_thesis_word_v8_pass1.docx");
const DRAFT_PDF = path.join(WORK_DIR, "main_thesis_word_v8_pass1.pdf");
const REPORT = path.join(OUT_DIR, "visual_check_report.md");
const MANIFEST = path.join(OUT_DIR, "main_thesis_word_v8_manifest.json");
...
function inlineRuns(text, opts = {}) {
  ...
  for (const part of parts) {
    ...
    if (part.startsWith("`") && part.endsWith("`")) {
      ...
      runs.push(...splitCitationRuns(part, {
        size,
        font,
        bold: opts.bold,
...
function collectChecks(pdfPath, tocDerivation, headings) {
  ...
  return {
    ...
    hasAppendix: allText.includes(normalizeMatchText("附录A 主要实验结果说明")),
...
function writeReport(checks, tocMap) {
  const lines = [
    ...
    "- V8 keeps signature lines blank because signatures are not required for this pass.",
...
async function main() {
  ...
  const formulas = collectFormulas(elements);
