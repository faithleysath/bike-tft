...
const SOURCE_MD = path.join(ROOT, "thesis", "main_thesis_v17", "thesis_draft.md");
const FIGURE_DIR = path.join(ROOT, "thesis", "main_thesis_v17", "figures");
...
const OUT_DOCX = path.join(OUT_DIR, "main_thesis_word_v19_final.docx");
const OUT_PDF = path.join(OUT_DIR, "main_thesis_word_v19_final.pdf");
const DRAFT_DOCX = path.join(WORK_DIR, "main_thesis_word_v19_pass1.docx");
const DRAFT_PDF = path.join(WORK_DIR, "main_thesis_word_v19_pass1.pdf");
const REPORT = path.join(OUT_DIR, "visual_check_report.md");
const MANIFEST = path.join(OUT_DIR, "main_thesis_word_v19_manifest.json");
...
function writeReport(checks, tocMap) {
  const lines = [
    ...
    "- V19 follows the Attachment 4 front-matter order: cover, declaration/authorization, table of contents, Chinese abstract, English abstract, and body.",
    "- V19 aligns the declaration and authorization pages to the Attachment 4 sample: red centered headings, KaiTi body text, and the prescribed authorization wording.",
    "- V19 rebuilds the cover information block with equal-width underline cells and centered values.",
    "- V19 inserts author and advisor electronic signature images and sets all signature dates to 2026年5月11日.",
    "- V19 keeps the Word automatic TOC field with cached page entries for PDF preview.",
    "- V19 keeps the level-1 TOC style and left-aligned references from v18.",
    "- V19 removes the short appendix section.",
