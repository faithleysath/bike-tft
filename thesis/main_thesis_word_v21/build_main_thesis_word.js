...
const SOURCE_MD = path.join(ROOT, "thesis", "main_thesis_v21", "thesis_draft.md");
const FIGURE_DIR = path.join(ROOT, "thesis", "main_thesis_v21", "figures");
const TEMPLATE = path.join(ROOT, "thesis", "thesis_template.dotx");
const OUT_DIR = __dirname;
const WORDMARK = path.join(OUT_DIR, "assets", "nuist_wordmark_red.png");
...
const SIGN_DATE = "2026年5月11日";

const AUTHOR_SIGNATURE = path.join(OUT_DIR, "assets", "signature_author_wu_tianyi.png");
const ADVISOR_SIGNATURE = path.join(OUT_DIR, "assets", "signature_advisor_hu_wei.png");

const OUT_DOCX = path.join(OUT_DIR, "main_thesis_word_v21_final.docx");
const OUT_PDF = path.join(OUT_DIR, "main_thesis_word_v21_final.pdf");
const DRAFT_DOCX = path.join(WORK_DIR, "main_thesis_word_v21_pass1.docx");
const DRAFT_PDF = path.join(WORK_DIR, "main_thesis_word_v21_pass1.pdf");
const REPORT = path.join(OUT_DIR, "visual_check_report.md");
const MANIFEST = path.join(OUT_DIR, "main_thesis_word_v21_manifest.json");
...
function paragraph(children, opts = {}) {
  ...
  return new Paragraph({
    ...
    overflowPunctuation: opts.overflowPunctuation,
  });
}
...
function extractWordmarkBuffer() {
  try {
    return fs.readFileSync(WORDMARK);
  } catch {
    return null;
  }
}
...
function fixedBorderCell(text, opts = {}) {
  const border = opts.border || { top: nilBorder(), bottom: nilBorder(), left: nilBorder(), right: nilBorder() };
  return new TableCell({
    width: { size: opts.width, type: WidthType.DXA },
    verticalAlign: VerticalAlign.CENTER,
    margins: {
      top: opts.marginTop === undefined ? 60 : opts.marginTop,
      bottom: opts.marginBottom === undefined ? 60 : opts.marginBottom,
      left: opts.marginLeft === undefined ? 60 : opts.marginLeft,
      right: opts.marginRight === undefined ? 60 : opts.marginRight,
    },
    borders: border,
    children: [
      paragraph([run(String(text), {
        size: opts.size || 24,
        font: opts.font || FONT_CN,
        bold: opts.bold,
        color: opts.color,
...
function coverFieldsTable(entries) {
  const tableWidth = 8000;
  ...
  return new Table({
    ...
    rows: entries.map((entry, index) =>
      new TableRow({
        children: [
          fixedBorderCell(entry.label, {
            width: labelWidth,
            size: entry.labelSize || 30,
            font: entry.labelFont || FONT_CN,
            alignment: AlignmentType.RIGHT,
            marginTop: index === 0 ? 72 : 48,
            marginBottom: 48,
          }),
          fixedBorderCell(entry.value, {
            width: valueWidth,
            size: entry.valueSize || 30,
            font: entry.valueFont || FONT_CN,
            bold: entry.boldValue,
            alignment: AlignmentType.CENTER,
            border: {
              top: nil,
              bottom: bottomBorder,
...
function signatureImage(file, width, height, altName) {
  return new ImageRun({
    type: "png",
    data: fs.readFileSync(file),
...
function signatureTable(label, imagePath, dateText, opts = {}) {
  const tableWidth = 8700;
  const labelWidth = 2300;
  const imageCellWidth = 2200;
  const imageRunWidth = opts.imageRunWidth || 120;
  const imageRunHeight = opts.imageRunHeight || 60;
  const dateLabelWidth = 1500;
  const dateValueWidth = tableWidth - labelWidth - imageCellWidth - dateLabelWidth;
  const bottomBorder = { style: BorderStyle.SINGLE, size: 4, color: "000000" };
  const nil = nilBorder();
  return new Table({
    width: { size: tableWidth, type: WidthType.DXA },
    columnWidths: [labelWidth, imageCellWidth, dateLabelWidth, dateValueWidth],
    layout: "fixed",
    alignment: AlignmentType.CENTER,
    borders: {
      top: nil,
      ...
      new TableRow({
        children: [
          ...
          new TableCell({
            ...
            borders: { top: nil, bottom: bottomBorder, left: nil, right: nil },
            children: [
              paragraph([
                signatureImage(imagePath, imageRunWidth, imageRunHeight, label),
              ], {
                alignment: AlignmentType.CENTER,
                spacing: { before: 0, after: 0, line: 220 },
              }),
...
function coverSection(logoBuffer, wordmarkBuffer) {
  const children = [
    paragraph([run("单位代码：", { size: 22 }), run("10300", { size: 22, underline: true })], {
      alignment: AlignmentType.LEFT,
      spacing: { before: 0, after: 330, line: 260 },
    }),
  ];
  if (wordmarkBuffer) {
    children.push(
      paragraph([
        new ImageRun({
          type: "png",
          data: wordmarkBuffer,
          transformation: { width: 420, height: 79 },
          altText: { title: "南京信息工程大学", description: "南京信息工程大学校名", name: "nuist_wordmark" },
        }),
      ], {
        alignment: AlignmentType.CENTER,
    ...
    children.push(center("南京信息工程大学", { size: 36, bold: true, font: FONT_HEI, spacing: { before: 0, after: 360, line: 360 } }));
  }
  children.push(
    center("本科毕业设计", { size: 36, bold: true, font: FONT_HEI, spacing: { before: 0, after: 320, line: 420 } }),
  );
  if (logoBuffer) {
    children.push(
      paragraph([
  ...
  children.push(
    coverFieldsTable([
      { label: "题目：", value: SHORT_TITLE, labelFont: FONT_HEI, valueFont: FONT_HEI, valueSize: 28, boldValue: true },
      { label: "学生姓名：", value: AUTHOR },
...
function statementTitle(text, opts = {}) {
  return paragraph([run(text, { size: 36, font: FONT_HEI, color: "FF0000" })], {
    alignment: AlignmentType.CENTER,
    spacing: opts.spacing || { before: 0, after: 0, line: 360 },
  });
}

function statementParagraph(text, opts = {}) {
  return paragraph([run(text, { size: 24, font: FONT_KAI })], {
    alignment: AlignmentType.LEFT,
    indent: { firstLine: opts.firstLine === undefined ? 480 : opts.firstLine },
    spacing: { before: opts.before || 0, after: opts.after || 0, line: 420 },
  });
}

function statementSignatureLine(text, opts = {}) {
  return paragraph(text, {
    alignment: AlignmentType.LEFT,
    indent: { firstLine: opts.firstLine === undefined ? 358 : opts.firstLine },
    spacing: { before: opts.before || 0, after: opts.after || 0, line: 420 },
  });
}
...
function declarationSection() {
  return {
    ...
    children: [
      emptyParagraph(360),
      statementTitle("郑  重  声  明", { spacing: { before: 0, after: 360, line: 360 } }),
      statementParagraph("本人以“求实 创新”的科学精神从事科学研究工作，所呈交的论文是我个人在导师指导下进行的研究工作及取得的研究成果。本论文尽我所知，所有测试、数据和相关材料均为真实有效；文中除引文和致谢内容外，未抄袭其他人或其他机构已经发表或撰写过的研究成果。与我一同工作同志对本研究所做的贡献均已在论文中作了声明并表示谢意。"),
      statementParagraph("本人毕业论文及涉及相关资料若有不实，愿意承担一切相关的法律责任。", { firstLine: 358 }),
      emptyParagraph(180),
      signatureTable("论文作者签名：", AUTHOR_SIGNATURE, SIGN_DATE, { imageRunWidth: 145, imageRunHeight: 59 }),
      emptyParagraph(360),
      statementTitle("论文使用授权说明", { spacing: { before: 0, after: 360, line: 360 } }),
      statementParagraph("本人授权南京信息工程大学可以保留并向国家有关部门或机构送交论文的复印件和电子文档；允许论文被查阅和借阅；可以将学位论文的全部或部分内容编入有关数据库进行检索；可以采用影印、缩印或扫描等复制手段保存、汇编本学位论文。不可用于任何非法用途。本文电子文档的内容和纸质论文的内容相一致。论文的公布（包括刊登）授权南京信息工程大学教务处办理。", { firstLine: 358, before: 312 }),
      emptyParagraph(160),
      signatureTable("论文作者签名：", AUTHOR_SIGNATURE, SIGN_DATE, { imageRunWidth: 145, imageRunHeight: 59 }),
      signatureTable("指导教师签名：", ADVISOR_SIGNATURE, SIGN_DATE, { imageRunWidth: 120, imageRunHeight: 81 }),
    ],
  };
}
...
function bodySection(elements) {
  const children = [];
  let inAcknowledgement = false;
  for (const element of elements) {
    if (element.type === "heading") {
      const compactHeading = element.text.replace(/\s+/g, "");
      inAcknowledgement = compactHeading === "致谢";
      children.push(headingParagraph(element.text, element.level));
    } else if (element.type === "paragraph") {
      const inReferenceList = /^\[\d+\]/.test(element.text.trim());
      if (inReferenceList) {
        children.push(referenceParagraph(element.text));
      } else {
        children.push(bodyParagraph(element.text, {
          font: inAcknowledgement ? FONT_KAI : FONT_CN,
        }));
      }
...
function makeDocument(parts, elements, headings, tocPageMap) {
  ...
  const wordmark = extractWordmarkBuffer();
  return new Document({
    creator: "Codex",
    title: SHORT_TITLE,
      ...
      paragraphStyles: [
        {
          ...
          next: "Normal",
          quickFormat: true,
          run: { size: 28, bold: true, font: FONT_HEI, color: "000000" },
        ...
        },
        {
          ...
          next: "Normal",
          quickFormat: true,
          run: { size: 24, bold: true, font: FONT_HEI, color: "000000" },
        ...
        },
        {
          ...
          next: "Normal",
          quickFormat: true,
          run: { size: 24, font: FONT_CN, color: "000000" },
...
function writeReport(checks, tocMap) {
  const lines = [
    ...
    "- V21 follows the Attachment 4 front-matter order: cover, declaration/authorization, table of contents, Chinese abstract, English abstract, and body.",
    "- V21 aligns the declaration and authorization pages to the Attachment 4 sample: red centered headings, KaiTi body text, and the prescribed authorization wording.",
    "- V21 keeps the cover field block with equal-width underline cells and centered values.",
    "- V21 keeps the supplied author and advisor electronic signatures and sets all signature dates to 2026年5月11日.",
    "- V21 keeps the Word automatic TOC field with cached page entries for PDF preview.",
    "- V21 keeps the level-1 TOC style and left-aligned references from v18.",
    "- V21 removes the short appendix section.",
    "- V21 replaces and expands Chapter 4 model architecture figures with six full-resolution gpt-image-2 diagrams.",
    "- V21 adds two Chapter 5 algorithm figures for rolling-horizon inventory planning and minimum-cost-flow matching.",
    "- V21 sets acknowledgement body paragraphs in KaiTi according to Attachment 4.",
...
function writeManifest(checks) {
  const manifest = {
    ...
    iteration_notes: [
      ...
      "V18 moves the table of contents before the Chinese and English abstracts according to Attachment 4 sample pages and user preference.",
      "V18 tightens TOC level-1 styling and reference paragraph alignment to better match the school template.",
      "V19 rebuilds the cover field block so underline widths are equal and field values are centered above the lines.",
      "V19 inserts the supplied author and advisor electronic signatures and sets signature dates to 2026年5月11日.",
      "V20 replaces and expands Chapter 4 model architecture figures with six full-resolution gpt-image-2 diagrams.",
      "V20 sets acknowledgement body paragraphs in KaiTi according to Attachment 4.",
      "V21 adds two Chapter 5 algorithm figures for rolling-horizon inventory planning and minimum-cost-flow matching.",
    ],
    checks,
    artifacts: {
