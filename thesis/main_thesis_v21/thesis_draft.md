...
图4-1展示了 AGCRN 基线模型的总体结构。

![图4-1 AGCRN 基线模型总体结构](figures/fig4_1_agcrn_overview.png)

图4-2展示了 AGCRN 单元与自适应图卷积细节。

![图4-2 AGCRN 单元与自适应图卷积细节](figures/fig4_2_agcrn_cell_avwgcn_detail.png)
...
图4-3展示了 Graph WaveNet（目标时间特征与净流量辅助损失）总体结构。

![图4-3 Graph WaveNet（目标时间特征与净流量辅助损失）总体结构](figures/fig4_3_graph_wavenet_overview.png)

图4-4展示了 Graph WaveNet 时空块与支撑图细节。

![图4-4 Graph WaveNet 时空块与支撑图细节](figures/fig4_4_graph_wavenet_block_support_detail.png)
...
图4-5展示了 TFT-style 分位数预测模块总体结构。

![图4-5 TFT-style 分位数预测模块总体结构](figures/fig4_5_tft_style_overview.png)

图4-6展示了 TFT-style 时间注意力与分位数输出细节。

![图4-6 TFT-style 时间注意力与分位数输出细节](figures/fig4_6_tft_style_attention_quantile_detail.png)
...
![图5-1 滚动时域库存规划算法流程](figures/fig5_1_rolling_horizon_inventory_planning.png)
...
![图5-2 最小费用流匹配算法流程](figures/fig5_2_min_cost_flow_matching.png)
...
![图5-3 预测驱动调度流程](figures/fig5_1_rebalancing_pipeline.png)
...
为了进一步观察不同模型在多步预测中的误差变化，本文选取 Graph WaveNet 基础模型、Graph WaveNet 改进模型和 TFT-style q50 结果，绘制 1 至 12 小时预测步长上的 MAE 曲线，如图7-5所示。可以看到，Graph WaveNet 基础模型在中后段 horizon 上波动较大，加入目标时间特征和净流量辅助损失后曲线更加平稳；TFT-style q50 在多数步长上保持最低误差，说明分位数训练和多源特征融合不仅改善整体平均指标，也改善了不同预测步长上的误差分布。

![图7-5 代表性模型分步长 MAE 曲线](figures/fig7_5_horizon_mae_curve.png)
...
PICP80 为 0.8107，说明 q10-q90 区间对真实值的覆盖率略高于 80%，整体校准较合理。平均区间宽度为 4.8970，说明模型在保持覆盖率的同时没有生成过宽区间。本文进一步选取典型站点和时段绘制 q10-q90 预测区间样例，用于展示分位数模型在具体时段中的不确定性表达。
...
为了更直观地说明调度动作如何改变单站库存轨迹，图7-6给出一个代表性站点在测试窗口局部片段中的库存回放。该站点在无调度基线下长期接近或高于安全库存上界，最小费用流调度通过连续移出车辆将库存拉回安全库存带附近；随后真实下一小时库存也基本围绕安全库存带上沿波动。该案例说明，本文调度算法并不是只在汇总指标上改善结果，而是能够在站点级别将未来净流量预测转化为可解释的取放动作。

![图7-6 单站库存回放与调度修正案例](figures/fig7_6_inventory_case_study.png)
...
本文从选题、数据整理、模型实验到系统实现的全过程，得到了指导教师胡伟老师的指导和帮助。感谢胡伟老师在毕业设计方向、技术路线和论文结构上的建议。感谢南京信息工程大学未来技术学院提供的学习环境和毕业设计管理支持。感谢开源社区提供的深度学习框架、地图可视化工具、数据处理工具和公开数据资源，使本文能够在可复现的基础上完成共享单车预测与调度系统。最后，感谢家人和同学在毕业设计期间给予的支持与鼓励。
