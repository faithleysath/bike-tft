...
const SOURCE_MD = path.join(ROOT, "thesis", "main_thesis_v20", "thesis_draft.md");
const FIGURE_DIR = path.join(ROOT, "thesis", "main_thesis_v20", "figures");
...
const OUT_DOCX = path.join(OUT_DIR, "main_thesis_word_v20_final.docx");
const OUT_PDF = path.join(OUT_DIR, "main_thesis_word_v20_final.pdf");
const DRAFT_DOCX = path.join(WORK_DIR, "main_thesis_word_v20_pass1.docx");
const DRAFT_PDF = path.join(WORK_DIR, "main_thesis_word_v20_pass1.pdf");
const REPORT = path.join(OUT_DIR, "visual_check_report.md");
const MANIFEST = path.join(OUT_DIR, "main_thesis_word_v20_manifest.json");
...
function writeReport(checks, tocMap) {
  const lines = [
    ...
    "- V20 follows the Attachment 4 front-matter order: cover, declaration/authorization, table of contents, Chinese abstract, English abstract, and body.",
    "- V20 aligns the declaration and authorization pages to the Attachment 4 sample: red centered headings, KaiTi body text, and the prescribed authorization wording.",
    "- V20 keeps the cover field block with equal-width underline cells and centered values.",
    "- V20 keeps the supplied author and advisor electronic signatures and sets all signature dates to 2026年5月11日.",
    "- V20 keeps the Word automatic TOC field with cached page entries for PDF preview.",
    "- V20 keeps the level-1 TOC style and left-aligned references from v18.",
    "- V20 removes the short appendix section.",
