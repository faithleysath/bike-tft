...
const SOURCE_MD = path.join(ROOT, "thesis", "main_thesis_v16", "thesis_draft.md");
const FIGURE_DIR = path.join(ROOT, "thesis", "main_thesis_v16", "figures");
...
const OUT_DOCX = path.join(OUT_DIR, "main_thesis_word_v16_final.docx");
const OUT_PDF = path.join(OUT_DIR, "main_thesis_word_v16_final.pdf");
const DRAFT_DOCX = path.join(WORK_DIR, "main_thesis_word_v16_pass1.docx");
const DRAFT_PDF = path.join(WORK_DIR, "main_thesis_word_v16_pass1.pdf");
const REPORT = path.join(OUT_DIR, "visual_check_report.md");
const MANIFEST = path.join(OUT_DIR, "main_thesis_word_v16_manifest.json");
...
function writeReport(checks, tocMap) {
  const lines = [
    ...
    "- V16 follows the Attachment 4 front-matter order: cover, declaration/authorization, table of contents, Chinese abstract, English abstract, and body.",
    "- V16 keeps the cover rebuilt against the Attachment 4 front-page sample, including the red NUIST wordmark and field layout.",
    "- V16 keeps the Word automatic TOC field with cached page entries for PDF preview.",
    "- V16 keeps justified reference entries and paragraph word wrap for long English words.",
    "- V16 removes the short appendix section and keeps signature lines blank because signatures are not required for this pass.",
...
function writeManifest(checks) {
  const manifest = {
    ...
    iteration_notes: [
      ...
      "V16 moves the table of contents before the Chinese and English abstracts according to Attachment 4.",
    ],
    checks,
    artifacts: {
