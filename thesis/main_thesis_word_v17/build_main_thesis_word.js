...
const OUT_DOCX = path.join(OUT_DIR, "main_thesis_word_v17_final.docx");
const OUT_PDF = path.join(OUT_DIR, "main_thesis_word_v17_final.pdf");
const DRAFT_DOCX = path.join(WORK_DIR, "main_thesis_word_v17_pass1.docx");
const DRAFT_PDF = path.join(WORK_DIR, "main_thesis_word_v17_pass1.pdf");
const REPORT = path.join(OUT_DIR, "visual_check_report.md");
const MANIFEST = path.join(OUT_DIR, "main_thesis_word_v17_manifest.json");
...
function coverSection(logoBuffer, wordmarkBuffer) {
  ...
  children.push(
    center("本科毕业设计", { size: 52, bold: true, font: FONT_HEI, spacing: { before: 0, after: 320, line: 520 } }),
  );
  if (logoBuffer) {
    children.push(
      paragraph([
  ...
  children.push(
    coverEntry("题    目：", SHORT_TITLE, { size: 32, labelFont: FONT_HEI, valueFont: FONT_HEI, boldValue: true, after: 300 }),
...
function writeReport(checks, tocMap) {
  const lines = [
    ...
    "- V17 follows the Attachment 4 front-matter order: cover, declaration/authorization, table of contents, Chinese abstract, English abstract, and body.",
    "- V17 aligns the declaration and authorization pages to the Attachment 4 sample: red centered headings, KaiTi body text, left-aligned signature lines, and the prescribed authorization wording.",
    "- V17 keeps the cover rebuilt against the Attachment 4 front-page sample, including the red NUIST wordmark and field layout.",
    "- V17 keeps the Word automatic TOC field with cached page entries for PDF preview.",
    "- V17 keeps justified reference entries and paragraph word wrap for long English words.",
    "- V17 removes the short appendix section and keeps signature lines blank because signatures are not required for this pass.",
...
function writeManifest(checks) {
  const manifest = {
    ...
    iteration_notes: [
      ...
      "V17 moves the table of contents before the Chinese and English abstracts according to Attachment 4.",
    ],
    checks,
    artifacts: {
